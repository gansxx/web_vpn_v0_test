...
export function useAndroidDownloadUrl() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        const data: AndroidDownloadData = JSON.parse(stored)
      ...
      console.warn("Failed to load Android download URL from session:", error)
      sessionStorage.removeItem(SESSION_KEY)
    }
  }, [])
  ...
  const storeDownloadUrl = useCallback((data: {
    ...
    try {
      const cacheData: AndroidDownloadData = {
      ...
      console.warn("Failed to store Android download URL in session:", error)
    }
  }, [])

  // Get cached download URL
  const getCachedDownloadUrl = useCallback((): AndroidDownloadData | null => {
    ...
    try {
      ...
      console.warn("Failed to get cached Android download URL:", error)
      return null
    }
  }, [])
