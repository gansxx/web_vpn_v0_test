...
export function useLinuxDownloadUrl() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        const data: LinuxDownloadData = JSON.parse(stored)
      ...
      console.warn("Failed to load Linux download URL from session:", error)
      sessionStorage.removeItem(SESSION_KEY)
    }
  }, [])
  ...
  const storeDownloadUrl = useCallback((data: {
    ...
    try {
      const cacheData: LinuxDownloadData = {
      ...
      console.warn("Failed to store Linux download URL in session:", error)
    }
  }, [])

  // Get cached download URL
  const getCachedDownloadUrl = useCallback((): LinuxDownloadData | null => {
    ...
    try {
      ...
      console.warn("Failed to get cached Linux download URL:", error)
      return null
    }
  }, [])
