...
export function useMacosDownloadUrl() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        const data: MacosDownloadData = JSON.parse(stored)
      ...
      console.warn("Failed to load macOS download URL from session:", error)
      sessionStorage.removeItem(SESSION_KEY)
    }
  }, [])
  ...
  const storeDownloadUrl = useCallback((data: {
    ...
    try {
      const cacheData: MacosDownloadData = {
      ...
      console.warn("Failed to store macOS download URL in session:", error)
    }
  }, [])

  // Get cached download URL
  const getCachedDownloadUrl = useCallback((): MacosDownloadData | null => {
    ...
    try {
      ...
      console.warn("Failed to get cached macOS download URL:", error)
      return null
    }
  }, [])
